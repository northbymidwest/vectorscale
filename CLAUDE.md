...
5. **init-positions** — Initializes the position texture from cell graph original positions.
6. **optimize-energy** (x2) — 2D Newton-Raphson minimizing curvature + positional energy with analytical gradient and Hessian (3 iterations). Stores absolute positions in F32 framebuffers.
...
- **Cell graph** (pass 3): Width = W+1 (corners), height = 6×(H+1) (3 rows × 2 slots per corner). Flags and direction packs are stored as raw floats in F32 textures, decoded with `uint(val + 0.5)`. Neighbor indices encoded as `(cx, cy, slot)`.
- **Positions** (passes 4-7): Width = W+1, height = 2×(H+1) (2 slots per corner). Stores absolute `(x, y)` positions in F32 framebuffers.
